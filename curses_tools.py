...
def draw_frame(canvas, start_row, start_column, text, negative=False):
    ...
    for row, line in enumerate(text.splitlines(), round(start_row)):
        if row < config.SCREEN_BORDER_WIDTH:
            continue

        if row >= rows_number - config.SCREEN_BORDER_WIDTH:
            break

        for column, symbol in enumerate(line, round(start_column)):
            if column < config.SCREEN_BORDER_WIDTH:
                continue

            if column >= columns_number - config.SCREEN_BORDER_WIDTH:
                break
